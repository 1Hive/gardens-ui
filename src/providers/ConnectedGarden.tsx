...
const ConnectedGardenContext = React.createContext<State | undefined | null>(null);
...
export function ConnectedGardenProvider({ children }) {
  ...
  return <ConnectedGardenContext.Provider value={null}>{children}</ConnectedGardenContext.Provider>;
}

function WithGarden({ children, gardenAddress }) {
  const { preferredNetwork } = useWallet();
  const { gardensMetadata } = useGardens();
  const [connectedGarden, connectedGardenLoading] = useGarden(gardenAddress, gardensMetadata, preferredNetwork);
      ...
      {connectedGarden ? (
        <Connect>
          <ActivityProvider>
            <GardenStateProvider>
              <StakingProvider>
                <AgreementSubscriptionProvider>{children}</AgreementSubscriptionProvider>
              </StakingProvider>
            </GardenStateProvider>
          </ActivityProvider>
        </Connect>
...
function useGarden(id, gardensMetadata, chainId) {
  ...
  useEffect(() => {
    ...
    const fetchGarden = async () => {
      ...
      try {
        const result = await getGarden({ network: chainId, subgraphUrl: subgraphs.gardens }, id);
