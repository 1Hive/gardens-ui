...
const DAOContext = React.createContext<State | undefined | null>(null);

export function GardensProvider({ children }) {
  const { preferredNetwork } = useWallet();
  const [queryFilters, filters] = useGardenFilters();
  const [gardens, gardensMetadata, gardensLoading, reload] = useGardensList(queryFilters, filters, preferredNetwork);
...
function useFilteredGardens(gardens, gardensMetadata, filters) {
  ...
  return useMemo(() => {
    const mergedGardens = gardens.map(garden => mergeGardenMetadata(garden, gardensMetadata));
    if (!debouncedNameFilter) {
      return mergedGardens;
    }
    return mergedGardens.filter(garden => testNameFilter(debouncedNameFilter, garden));
...
function useGardensList(queryFilters, filters, chainId) {
  ...
  const [gardensMetadata, loadingMetadata] = useGardensMetadata(refetchTriger, chainId);
  ...
  useEffect(() => {
    ...
    const fetchGardens = async () => {
      try {
        const result = await getGardens({ network: chainId, subgraphUrl: subgraphs.gardens }, { ...sorting.queryArgs });

        setGardens(result.filter(garden => !getVoidedGardensByNetwork().get(garden.id)));
      } catch (err) {
        setGardens([]);
