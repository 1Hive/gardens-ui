import { getNetworkType, isLocalOrUnknownNetwork, getNetworkName } from '@utils/web3-utils';
...
const networks = {
  rinkeby: {
    ...
    subgraphs: {
      agreement: 'https://api.thegraph.com/subgraphs/name/1hive/agreement-rinkeby',
  ...
  polygon: {
    ...
    subgraphs: {
      agreement: 'https://api.thegraph.com/subgraphs/name/1hive/agreement-polygon',
    ...
    eip3085: {
      ...
      iconUrls: ['https://raw.githubusercontent.com/maticnetwork/polygon-token-assets/main/icons/matic.svg'],
...
export function getAgreementConnectorConfig(chainId: any) {
  const agreementSubgraph = getNetwork(chainId)?.subgraphs?.agreement;
  return {
    agreement: agreementSubgraph && ['thegraph', { subgraphUrl: agreementSubgraph }],
  };
}

export const SUPPORTED_CHAINS = [4, 100, 137]; // Add  arbitrum  chains id + fill the network json with the data

export function isSupportedChain(chainId: any) {
