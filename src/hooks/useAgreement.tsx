...
export function useAgreement() {
  ...
  useEffect(() => {
    ...
    async function processAgreementDetails() {
      const { currentVersion, appsWithRequirements, signer, stakingFactory } = agreement;
      const { content, effectiveFrom, title, versionId } = currentVersion;

      const signatures = signer ? await signer.signatures() : [];

      const hasSignedLast = signer && (await signer.hasSigned(currentVersion.versionId));

      const disputableAppsWithRequirements = processDisputableApps(apps, appsWithRequirements);

      const lastSignatureDate = signatures.length > 0 ? signatures[signatures.length - 1].createdAt * 1000 : null;
...
function processDisputableApps(apps, disputableApps) {
  ...
  const processedDisputableApps = disputableApps.map(disputableApp => {
    const { address: disputableAppAddress, actionAmount, challengeDuration, challengeAmount, token } = disputableApp;

    const targetApp = apps.find(({ address }) => addressesEqual(address, disputableAppAddress));
