...
const getPostDateTime = post => new Date(post.created_at).toLocaleString('en-US')
const getPostDate = post => new Date(post.created_at).toLocaleDateString('en-US')

function DiscourseComments({ topicId }) {
  const connectedGarden = useConnectedGarden()
  const [posts, setPosts] = useState([])

  useEffect(() => {
    fetch(`${MIDDLEWARE_ENDPOINT}/cors/${connectedGarden.forumURL}/t/${topicId}.json`)
