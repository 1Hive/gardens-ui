...
const GardensFilters = ({ itemsSorting, nameFilter, sortingFilter, onNameFilterChange, onSortingFilterChange }) => {
  ...
  return (
    ...
    >
      <FilterItem>
        <DropDown items={supportedChains} onChange={onPreferredNetworkChange} selected={selectedIndex} wide />
      </FilterItem>
      <FilterItem>
        <DropDown
          header="Sort by"
      ...
      <FilterItem grow={1.3}>
        <SearchInput value={nameFilter} onChange={onNameFilterChange} placeholder="Search by Garden name" wide />
      </FilterItem>
    </div>
  );
};
