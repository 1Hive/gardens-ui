...
import { GU, IconCheck, IconCross, LoadingRing, Tag, textStyle, Timer, useLayout, useTheme } from '@1hive/1hive-ui';
...
export function ConvictionChart({ proposal, withThreshold = true, lines }) {
  ...
  return (
    <LineChart
      ...
      captionsHeight={20}
      color={i => [theme.info.toString(), theme.infoSurfaceContent.toString()][i]}
      threshold={withThreshold && !Number.isNaN(threshold) && threshold && normalize(threshold)}
    />
  );
}
...
export function ConvictionBar({ proposal, withThreshold = true }) {
  ...
  return (
    <div>
      ...
      <div>
        <span>
          {stakedConviction.eq(0) ? '0' : stakedConviction.multipliedBy(new BigNumber('100')).toFixed(2)}%
          ...
          {!signalingProposal &&
            (withThreshold ? (
              ...
              >
                {neededConviction
                  ? `(at least ${neededConviction.multipliedBy(new BigNumber('100')).toFixed(2)}% needed)`
              ...
              <span
                ...
                `}
              >
                {Math.round(stakedConviction * 100) !== Math.round(futureStakedConviction * 100)
...
export function ConvictionCountdown({ proposal, shorter }) {
  ...
  const { currentConviction, loading, neededTokens, statusData, threshold } = proposal;
  ...
  return loading ? (
    ...
    >
      {view === UNABLE_TO_PASS ? (
        <NegativeOutcome maxRatio={maxRatio} neededTokens={neededTokens} shorter={shorter} stakeToken={stakeToken} />
      ) : view === CHALLENGED ? (
        <Outcome
          result="Challenged"
          color="#F5A623"
...
export function ConvictionTrend({ proposal }) {
  ...
  return (
    <TrendWrapper compactMode={compactMode} color={theme.contentSecondary.toString()}>
      <TrendArrow>{convictionTrend > 0 ? '↑' : convictionTrend < 0 ? '↓' : '↝'}</TrendArrow>
      <span
        css={css`
          ${textStyle('body3')}
        `}
...
const PositiveOutcome = ({ endDate, shorter, view }) => {
  ...
  const text = view === MAY_PASS ? 'May pass' : view === EXECUTED ? 'Passed and executed' : 'Available for execution';

  return (
    <>
      <Outcome result={text} color={theme.positive.toString()} icon={<IconCheck />} />
...
const NegativeOutcome = ({ maxRatio, neededTokens, shorter, stakeToken }) => {
  ...
  return (
    <>
      <Outcome result="Won't pass" color={theme.negative.toString()} icon={<IconCross />} />
      {!shorter && (
        <>
          <span
            css={css`
            ...
            `}
          >
            {isNaN(neededTokens) ? 'Not enough funds in the organization' : 'Not enough support received'}
          </span>
          <span
            css={css`
          ...
          >
            (
            {isNaN(neededTokens) ? (
              `Funding requests must be below ${maxRatio * 100}% of organization total funds`
            ) : (
              <React.Fragment>
                At least <Tag>{`${formatTokenAmount(neededTokens, stakeToken.decimals)} ${stakeToken.symbol}`}</Tag>{' '}
                more needed
              </React.Fragment>
            )}
            ).
