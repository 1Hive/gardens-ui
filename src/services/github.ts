...
const ASSETS_FOLDER_BASE = 'https://raw.githubusercontent.com/1Hive/dao-list/master/assets';
...
export async function publishNewDao(daoAddress, daoMetadata) {
  try {
    ...
    newDaoList.push({
      ...
      logo: daoMetadata.logo && `${ASSETS_FOLDER_BASE}/${daoMetadata.name}/logo.${daoMetadata.logo.imageExtension}`,
      logo_type:
    ...
    const { data: commitSha } = await createCommit(latestCommitSha, newTreeSha, daoMetadata.name);
